...
export async function loginWithBadge(formData: FormData): Promise<LoginResult> {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('rlrdd_login_user', {
      p_badge: badgeNumber,
      p_password: password,
    });
    ...
    if (error) {
      const rpcMissing =
        error.code === 'PGRST202' ||
        error.message?.includes('rlrdd_login_user') ||
        error.details?.includes('rlrdd_login_user');

      if (rpcMissing) {
        return {
          ok: false,
          message:
            'Login not set up yet. Run sql/001_rlrdd_auth.sql in Supabase SQL Editor, then try again.',
        };
      }
    ...
    if (!canSignInToRlrdd(result.access_page) || !canAccessRlrdd({ role: result.role, access_page: result.access_page })) {
...
export async function logoutUser() {
  ...
  if (sessionToken) {
    try {
      const supabase = await createClient();
      await supabase.rpc('rlrdd_logout_user', { p_session: sessionToken });
    } catch {
      if (hasAdminClient()) {
        try {
          ...
          await admin
            .from(USERS_TABLE)
            .update({ rlrdd_session: null, updated_at: new Date().toISOString() })
            .eq('rlrdd_session', sessionToken);
        } catch {
