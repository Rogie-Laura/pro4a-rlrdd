...
function canManageUser(
  actorRole: string,
  target: { role: string; access_page: AccessPage }
): boolean {
  if (actorRole === 'RLRDD_admin' && target.access_page !== 'RLRDD') {
    return false;
  }

  return canManageTargetRole(actorRole, target.role as RlrddRole);
}
...
async function requireActor() {
  ...
  if (actorRole !== 'super_admin' && actorRole !== 'RLRDD_admin') {
...
export async function createManagedUser(formData: FormData): Promise<UserActionResult> {
  ...
  const accessPageRaw = String(formData.get('access_page') ?? 'RLRDD');
  const accessPage: AccessPage = isAccessPage(accessPageRaw) ? accessPageRaw : 'RLRDD';
  ...
  if (!canManageTargetRole(actorRole, role as RlrddRole)) {
...
export async function updateManagedUser(formData: FormData): Promise<UserActionResult> {
  ...
  if (!canManageUser(actorRole, target as { role: string; access_page: AccessPage })) {
...
export async function getAssignableRolesForActor(): Promise<RlrddRole[]> {
  const session = await requireActor();
