...
function canEditUser(actorRole: RlrddRole, user: ManagedUser): boolean {
  if (actorRole === 'RLRDD_admin' && user.access_page !== 'RLRDD') {
    return false;
  }

  return canManageTargetRole(actorRole, user.role as RlrddRole);
}
...
export function UserManagement({
  ...
  const defaultAccessPage = assignableAccessPages[0] ?? 'RLRDD';
  const defaultRole = rolesForAccessPage(defaultAccessPage)[0] ?? 'stn_logistics';

  const [addRank, setAddRank] = useState('');
  const [addOffice, setAddOffice] = useState('');
  const [addUnit, setAddUnit] = useState('');
  const [addAccessPage, setAddAccessPage] = useState<AccessPage>(defaultAccessPage);
  const [addRole, setAddRole] = useState<RlrddRole>(defaultRole);
  const [editRank, setEditRank] = useState('');
  const [editOffice, setEditOffice] = useState('');
  const [editUnit, setEditUnit] = useState('');
  const [editAccessPage, setEditAccessPage] = useState<AccessPage>('RLRDD');
  const [editRole, setEditRole] = useState<RlrddRole>('stn_logistics');
  ...
  useEffect(() => {
    if (!addRoleOptions.includes(addRole)) {
      setAddRole(addRoleOptions[0] ?? 'stn_logistics');
    }
  }, [addRole, addRoleOptions]);

  useEffect(() => {
    if (!editRoleOptions.includes(editRole)) {
      setEditRole(editRoleOptions[0] ?? 'stn_logistics');
    }
  ...
  useEffect(() => {
    ...
    setEditRole(editUser.role as RlrddRole);
  }, [editUser]);
  ...
  function closeAddModal() {
    ...
    setAddOffice('');
    setAddUnit('');
    setAddAccessPage(defaultAccessPage);
    setAddRole(rolesForAccessPage(defaultAccessPage)[0] ?? 'stn_logistics');
  ...
  function handleAddAccessPageChange(value: AccessPage) {
    ...
    setAddRole(rolesForAccessPage(value)[0] ?? 'stn_logistics');
  }

  function handleEditAccessPageChange(value: AccessPage) {
    setEditAccessPage(value);
    setEditRole(rolesForAccessPage(value)[0] ?? 'stn_logistics');
  }
  ...
  function requestCreateConfirm() {
    ...
    const accessPageValue = String(formData.get('access_page') ?? 'RLRDD');
      ...
      <div className="mb-3 flex shrink-0 flex-wrap items-center gap-x-3 gap-y-2">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="h-8 w-52 rounded-md border border-[var(--app-border)] bg-[var(--app-surface-2)] px-2.5 text-xs text-[var(--app-text)] outline-none focus:border-emerald-500/50"
          />
        </div>
        ...
        <button
          ...
          className="ml-auto inline-flex h-8 shrink-0 items-center rounded-md bg-emerald-500 px-3 text-xs font-semibold text-slate-950 transition hover:bg-emerald-400"
        >
          Add User
        </button>
      </div>
              ...
              filteredUsers.map((user, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`${tableCellClass} ${tableTruncateClass} font-medium text-emerald-700 dark:text-emerald-100`}
        ...
        >
          <form ref={createFormRef} id="create-user-form" className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <select id="role" name="role" required value={addRole} onChange={(event) => setAddRole(event.target.value as RlrddRole)} className={inputClass}>
          ...
          {lookup.offices.length === 0 ? (
            <p className="mt-3 text-xs text-emerald-600 dark:text-emerald-200">
              Office and unit lists are empty. Import personnel data in RPRMD first, or run{' '}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  value={editRole}
                  disabled={editUser.id === currentUserId}
                  onChange={(event) => setEditRole(event.target.value as RlrddRole)}
                  className={inputClass}
                >
                  {(editUser.id === currentUserId ? [editUser.role as RlrddRole] : editRoleOptions).map((role) => (
                    <option key={role} value={role}>{roleLabel(role)}</option>
                  ))}
                </select>
              </div>
