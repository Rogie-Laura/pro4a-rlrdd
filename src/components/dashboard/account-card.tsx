...
function AccountAvatar({ name, size = 'md' }: { name: string; size?: 'md' | 'lg' }) {
  ...
  return (
    <div
      className={`flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-teal-600 font-bold text-slate-950 ring-2 ring-emerald-500/20 ${sizeClass}`}
      aria-hidden="true"
    >
      {getInitials(name)}
    </div>
  ...
  return (
    <button
      ...
      className={`w-full px-3 py-2 text-left text-xs transition ${
        active
          ? 'bg-emerald-500/15 font-medium text-emerald-600 dark:text-emerald-200'
  ...
  return (
    ...
    >
      <button
        ...
        className="flex w-full items-start gap-2.5 rounded-lg border border-[var(--app-border)] bg-[var(--app-surface-2)] px-2.5 py-2 text-left transition hover:border-emerald-500/40"
      >
