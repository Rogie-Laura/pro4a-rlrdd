...
const fieldClass =
  'h-8 w-full rounded-md border border-[var(--app-border)] bg-[var(--app-surface-2)] px-2.5 text-xs text-[var(--app-text)] outline-none focus:border-emerald-500/50';
...
export function CommandIntegration({ apiKeys, baseUrl, hasServiceRole }: CommandIntegrationProps) {
  ...
  const vehiclesUrl = `${baseUrl}/api/command/vehicles`;
    ...
    <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-xs text-[var(--app-text-muted)]">
          ...
          vehicle data and analytics. Send the key in an{' '}
      ...
      <div className="mt-5 border-t border-[var(--app-border)] pt-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            disabled={!hasServiceRole || isPending}
            className="inline-flex h-8 shrink-0 items-center rounded-md bg-emerald-500 px-4 text-xs font-semibold text-slate-950 transition hover:bg-emerald-400 disabled:pointer-events-none disabled:opacity-50"
          >
